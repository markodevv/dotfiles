function b32
tc_buffer_is_in_project(Application_Links *app, Buffer_ID buffer)
{
	Scratch_Block scratch(app);
	Variable_Handle prj_var = vars_read_key(vars_get_root(), vars_save_string_lit("prj_config"));
	if (vars_is_nil(prj_var)) return false;
	String8 prj_dir = prj_path_from_project(scratch, prj_var);
	String8 buffer_name = push_buffer_file_name(app, scratch, buffer);
	buffer_name = string_replace(scratch, buffer_name, SCu8("\\"), SCu8("/"), StringFill_NoTerminate);
	return buffer_name.size != string_find_first(buffer_name, prj_dir, StringMatch_Exact);
}
...
tc_generate_all_buffers_list(Application_Links *app, Lister *lister){
	...
	if (!vars_is_nil(prj_var))
	{
		for (Buffer_ID buffer = get_buffer_next(app, 0, Access_Always);
			...
			if (tc_buffer_is_in_project(app, buffer))
			{
	...
	for (Buffer_ID buffer = get_buffer_next(app, 0, Access_Always);
		 ...
		 buffer = get_buffer_next(app, buffer, Access_Always)){
		for (i32 i = 0; i < viewed_buffer_count; i += 1){
			if (buffer == viewed_buffers[i] || tc_buffer_is_in_project(app, buffer)){
				goto skip2;
			}
		}
