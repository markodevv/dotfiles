    ...
    while (tokens.tokens[idx].pos < param_range.max)
    {
        ...
        switch (tokens.tokens[idx].sub_kind)
        {
            ...
            case TokenOdinKind_ParenOp:
            {
                ...
                Code_Index_Nest *_nest = code_index_get_nest(note->file, tokens.tokens[idx].pos+1);
                idx = token_index_from_pos(&tokens, _nest->close.max);
                goto type_postfix;
            }
        ...
        if (name.str) param->name = push_string_copy(arena, name);
        if (prefix.str) param->prefix = push_string_copy(arena, prefix);
        if (postfix.str) param->postfix = push_string_copy(arena, postfix);
        if (type.str) param->type = push_string_copy(arena, type);
        if (default_value.str) param->default_value = push_string_copy(arena, default_value);
